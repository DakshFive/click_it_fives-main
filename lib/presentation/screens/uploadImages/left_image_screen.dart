...
class LeftImageScreen extends StatefulWidget {
  final String? gtin;
  LeftImageScreen({Key? key, required this.gtin}) : super(key: key);
...
class _LeftImageScreenState extends State<LeftImageScreen>
    with AutomaticKeepAliveClientMixin<LeftImageScreen> {
  File? productImage;
  ...
  Future<Null> pickImage(
    ...
    try {
      ...
      try {
        ...
        if (imageResolution!.toLowerCase() == 'low') {
          ...
          if (AppPreferences.getValueShared('fetched_left_image') == '' ||
            ...
            productImage =
                File(AppPreferences.getValueShared('fetched_left_image'));
            /*editedSavedImage =
                File(AppPreferences.getValueShared('fetched_left_image'));*/
          }
          setState(() {});
      ...
      try {
        ...
        AppPreferences.addSharedPreferences(true, ClickItConstants.leftImageUploadedKey);
        AppPreferences.addSharedPreferences(true, 'isImageUploaded');
        AppPreferences.addSharedPreferences(widget.gtin!, 'gtin');
        AppPreferences.addSharedPreferences(productImage!.path, 'left_image');
  ...
  void getSavedData() async {
    ...
    // get the saved left image from the api
    ...
    if (response.statusCode == 200) {
      print('++');
      String leftImage =
          jsonDecode(await response.stream.bytesToString())['image_left'];
      print(leftImage);
      if (leftImage == '') {
        ...
        final editedImagePath =
            await AppPreferences.getValueShared('left_edited_image') == ''
                ? null
                : AppPreferences.getValueShared('left_edited_image');
        ...
        if (productImage == null) {
          ...
          await AppPreferences.addSharedPreferences(imagePath, 'left_image');
          await AppPreferences.addSharedPreferences(
              imagePath, 'fetched_left_image');
          await AppPreferences.addSharedPreferences(
              imagePath, 'left_edited_image');
          await getLocalSavedData();
