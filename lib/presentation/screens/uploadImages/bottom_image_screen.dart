...
class BottomImageScreen extends StatefulWidget {
  final String? gtin;
  BottomImageScreen({Key? key, required this.gtin}) : super(key: key);
...
class _BottomImageScreenState extends State<BottomImageScreen>
    with AutomaticKeepAliveClientMixin<BottomImageScreen> {
  File? productImage;
  ...
  Future<Null> pickImage(
    ...
    try {
        ...
        if (imageResolution!.toLowerCase() == 'low') {
         ...
         // ProgressLoader.hide();
          ...
          if (AppPreferences.getValueShared('fetched_bottom_image') == '' ||
            ...
            productImage =
                File(AppPreferences.getValueShared('fetched_bottom_image'));
            /*editedSavedImage =
                File(AppPreferences.getValueShared('fetched_bottom_image'));*/
          }
          setState(() {});
      ...
      try {
        ...
        AppPreferences.addSharedPreferences(productImage!.path, 'bottom_image');
  ...
  void getSavedData() async {
    ...
    // get the saved bottom image from the api
    ...
    if (response.statusCode == 200) {
      print('++');
      String bottomImage =
          jsonDecode(await response.stream.bytesToString())['image_bottom'];
      print(bottomImage);
      if (bottomImage == '') {
        ...
        final editedImagePath =
            await AppPreferences.getValueShared('bottom_edited_image') == ''
                ? null
                : AppPreferences.getValueShared('bottom_edited_image');
        ...
        if (productImage == null) {
          ...
          await AppPreferences.addSharedPreferences(imagePath, 'bottom_image');
          await AppPreferences.addSharedPreferences(
              imagePath, 'fetched_bottom_image');
          await AppPreferences.addSharedPreferences(
              imagePath, 'bottom_edited_image');
          await getLocalSavedData();
