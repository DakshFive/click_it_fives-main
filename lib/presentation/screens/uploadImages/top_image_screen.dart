...
class TopImageScreen extends StatefulWidget {
  final String? gtin;
  TopImageScreen({Key? key, required this.gtin}) : super(key: key);

  @override
  State<TopImageScreen> createState() => _TopImageScreenState();
}

class _TopImageScreenState extends State<TopImageScreen>
    with AutomaticKeepAliveClientMixin<TopImageScreen> {
  File? productImage;
  ...
  Future<Null> pickImage(
    ...
    try {
        ...
        if (imageResolution!.toLowerCase() == 'low') {
         ...
         // ProgressLoader.hide();
          ...
          if (AppPreferences.getValueShared('fetched_top_image') == '' ||
            ...
            productImage =
                File(AppPreferences.getValueShared('fetched_top_image'));
            /*editedSavedImage =
                File(AppPreferences.getValueShared('fetched_top_image'));*/
          }
          setState(() {});
      ...
      try {
        ...
        AppPreferences.addSharedPreferences(true, ClickItConstants.topImageUploadedKey);
        AppPreferences.addSharedPreferences(true, 'isImageUploaded');
        AppPreferences.addSharedPreferences(widget.gtin!, 'gtin');
        AppPreferences.addSharedPreferences(productImage!.path, 'top_image');
  ...
  void getSavedData() async {
    ...
    // get the saved top image from the api
    ...
    if (response.statusCode == 200) {
      print('++');
      String topImage =
          jsonDecode(await response.stream.bytesToString())['image_top'];
      print(topImage);
      if (topImage == '') {
        ...
        final editedImagePath =
            await AppPreferences.getValueShared('top_edited_image') == ''
                ? null
                : AppPreferences.getValueShared('top_edited_image');
        ...
        if (productImage == null) {
          ...
          await AppPreferences.addSharedPreferences(imagePath, 'top_image');
          await AppPreferences.addSharedPreferences(
              imagePath, 'fetched_top_image');
          await AppPreferences.addSharedPreferences(
              imagePath, 'top_edited_image');
          await getLocalSavedData();
