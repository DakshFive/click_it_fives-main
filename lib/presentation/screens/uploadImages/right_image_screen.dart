...
class RightImageScreen extends StatefulWidget {
  final String? gtin;
  RightImageScreen({Key? key, required this.gtin}) : super(key: key);
...
class _RightImageScreenState extends State<RightImageScreen>
    with AutomaticKeepAliveClientMixin<RightImageScreen> {
  File? productImage;
    ...
    try {
        ...
        if (imageResolution!.toLowerCase() == 'low') {
         ...
         // ProgressLoader.hide();
          ...
          if (AppPreferences.getValueShared('fetched_right_image') == '' ||
            ...
            productImage =
                File(AppPreferences.getValueShared('fetched_right_image'));
            /*editedSavedImage =
               File(AppPreferences.getValueShared('fetched_right_image'));*/
          }
          setState(() {});
      ...
      try {
       ...
       // backgroundRemovedImageBackup = productImage?.readAsBytesSync();
        ...
        AppPreferences.addSharedPreferences(true, ClickItConstants.rightImageUploadedKey);
        AppPreferences.addSharedPreferences(true, 'isImageUploaded');
        AppPreferences.addSharedPreferences(widget.gtin!, 'gtin');
        AppPreferences.addSharedPreferences(productImage!.path, 'right_image');
  ...
  void getSavedData() async {
    ...
    // get the saved right image from the api
    ...
    if (response.statusCode == 200) {
      print('++');
      String rightImage =
          jsonDecode(await response.stream.bytesToString())['image_right'];
      print(rightImage);
      if (rightImage == '') {
        ...
        final editedImagePath =
            await AppPreferences.getValueShared('right_edited_image') == ''
                ? null
                : AppPreferences.getValueShared('right_edited_image');
        ...
        if (productImage == null) {
          ...
          await AppPreferences.addSharedPreferences(imagePath, 'right_image');
          await AppPreferences.addSharedPreferences(
              imagePath, 'fetched_right_image');
          await AppPreferences.addSharedPreferences(
              imagePath, 'right_edited_image');
          await getLocalSavedData();
